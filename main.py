...
def main(page: ft.Page):
    ...
    if float(data) > float(__version__):
        ...
        dlg_modal = ft.AlertDialog(modal=True, title=ft.Text("Обновить программу?"),
                                   content=ft.Text('В этой версии добавлено обновление этикеток а5'),
                                   actions=[ft.TextButton("Обновить", on_click=update_dlg),
                                            ft.TextButton("Выход", on_click=close_dlg), ],
        ...
        page.add(ft.Row([ft.WindowDragArea(ft.Container(
                ft.ElevatedButton("Обновить", bgcolor=ft.colors.RED, on_click=open_dlg_modal, expand=True), ))]), )
    # Добавление картинки
